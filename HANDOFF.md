...
- **2026-08-30 バックフィルを有効化**: IN/BR の初回取得が 08-26 に完了し以後毎日更新されていることを
  ...
  （同日 21:24 JST の run から実際に始動・active:true を確認済み）。
- **2026-08-30 公式急上昇の毎時合流（`chart` ジョブ）**: 「200万再生の動画がランキングに無い」への
  対策。search が取りこぼす大物（実測: 急上昇の363万回が 24h に不在）を、chart=mostPopular
  （6国×1 unit・毎時144 u/日）で 24h/週間/月間へ「追加と統計更新だけ」合流させる
  （`scripts/lib/chart.mjs`）。予算は定常 7,823/日・バックフィル併走中 7,981/日で degraded なし。
  **書き手が2つあるリスト（24h/週間/月間×総合）は search 側も同じ合流規則を通す**
  （`isMultiWriterList`）。素の上書きだと chart だけが知る大物が完走のたびに消え、週間・月間では
  戻らないため（公開前の多角レビューで確定した指摘。詳細は DECISIONS 2026-08-30）。テスト274件緑。
  「いずれ非公式（独自）でちゃんと集計したい」は BACKLOG の機能欄に布石を書いた。
- ⚠ **ソフト上限 8,000 に対する余裕が 33 units しかない**（chart 追加前は 177）。
  次に何かを足すときは planner の降格に頼らず、増枠（ゲートF）とセットで考えること。
- ℹ **2026-08-30 深夜の時点で 24h/週間リストが薄い**（JP 24h 45件・JP 週間 55件・US 24h 27件）。
  chart 導入の最初の1回だけ毎時側でも剪定してしまい、窓の外に出ていた古い行が一掃されたため
  （＝公開中の「24h」に2日前の行が混ざっていたことが可視化された）。剪定は search 側だけに
  直したので**もう痩せない**。次の top24h / weekmonth の完走（クォータ日リセットは JST 16:00）で
  自然に埋まる。埋まらないままなら planner のログを見ること。
