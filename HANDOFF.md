...
最終更新: 2026-08-25（5回目） / 公開URL: **https://guroriya.github.io/yt-trend-app/** / 直近: `6784617`
...
- 収集は予算内で数日かけて埋まる（**240本が4日で出そろう**見込み。1日目70→2日目151→3日目222→4日目240）。
  **未収集の軸は隠さず「集計中」（点線＋…）として出す**（隠すと機能が消えたように見えるという発注者指摘）。
- 8/25 は割当を使い切って停止したが、再開カーソルを記録して後工程（ワード・伸び）は完走している（本番ログで確認）。
